...
**IMPORTANT**: Updating or adding tests is a **required step** in every plan that involves frontend changes. Never skip this.

When planning any frontend task (new view, component, service, store module, or router change):

1. **Always include an "Update / Add Tests" step** in the plan
2. Write or update tests **before or alongside** the implementation (not after)
3. Every new Vue component, service call, or Vuex/Pinia action **must** have corresponding tests
4. Tests live alongside their subjects or in a `__tests__/` directory next to the relevant file
5. After writing tests, always run them to confirm they pass:
   ```powershell
   cd E:\learn\badminton-app\frontend
   npm run test:unit
   ```
...
- ✅ Expected UI elements are present for each state (loading, success, error)
...
#### Example Test Structure (Vue Test Utils + Jest pattern)
```javascript
